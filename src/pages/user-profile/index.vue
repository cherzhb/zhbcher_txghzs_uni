      ...
      <view class="section">
        <view class="glass-card">
          ...
          <view class="form-item">
            ...
            <picker mode="date" :value="birthDate" start="1940-01-01" :end="todayDate" @change="onDateChange">
              <view class="form-value-wrap">
                <text class="form-value">{{ birthDate || '请选择' }}</text>
                <text class="arrow">›</text>
              </view>
            </picker>
          </view>
...
const openGenderPicker = () => {
  uni.showActionSheet({
    itemList: ['男', '女'],
    success: (res) => { gender.value = res.tapIndex === 0 ? 1 : 2 }
  })
}
...
const openJobTypePicker = () => {
  uni.showActionSheet({
    ...
    success: (res) => { jobType.value = jobTypes[res.tapIndex].value }
  })
}
...
const openYearsPicker = () => {
  ...
  uni.showActionSheet({
    itemList: years,
    success: (res) => { contributionYears.value = res.tapIndex }
  })
}
...
const openIndexPicker = () => {
  ...
  uni.showActionSheet({
    itemList: indexes,
    success: (res) => { contributionIndex.value = 0.6 + res.tapIndex * 0.1 }
  })
}
