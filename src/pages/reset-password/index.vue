<template>
  <view class="reset-page">
        ...
        <template v-if="step === 1">
          ...
          <view class="form-group">
            ...
            <view class="code-row">
              ...
              <button class="btn-code" :disabled="countdown > 0 || sendingCode" @click="sendCode">
    ...
    <view class="tips-section">
      <text class="tips-text">• 验证码将发送到您的注册邮箱</text>
      <text class="tips-text">• 验证码5分钟内有效</text>
      <text class="tips-text">• 如果没有收到邮件，请检查垃圾邮件</text>
    </view>
  </view>
</template>
...
const sendCode = async () => {
  ...
  try {
    ...
    if (res.statusCode === 200) {
      ...
      const errorMsg = res.data?.error || '发送失败，请稍后重试'
...
const handleReset = async () => {
  ...
  try {
    ...
    if (res.statusCode === 200 && res.data && res.data.message) {
      ...
      const errorMsg = res.data?.error || '重置失败，请稍后重试'
      uni.showToast({ title: errorMsg, icon: 'none', duration: 3000 })
    }
  } catch (err) {
    uni.hideLoading()
